...
describe('On coffee photo retrieval asked', () => {
    ...
    it('should , in case of error, dispatch empty array', async () => {
        photoGateway.willFail = true
        await store.dispatch<any>(onCfPhotoRetrieval())
        const state = store.getState() as any
        expect((state.pState as AppStateWl["cfPhotos"]).byCoffeeId["15626614-f66d-4a41-b119-e27c9f963ab1"]).toBeUndefined()
        expect((state.pState as AppStateWl["cfPhotos"]).byCoffeeId).toEqual({})
    })
});