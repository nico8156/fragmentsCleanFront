...
- **Catalogue distant** : la recherche est exécutée par `GET /api/coffees?query=&cursor=&limit=`. Le gateway expose un résultat discriminé `updated | not-modified`, conserve l'ETag du snapshot et lit le curseur opaque dans `X-Next-Cursor`. Un `304` garde le cache Redux existant au lieu de devenir une erreur ou une liste vide.
- **Fraîcheur SSE** : un `projection.updated/coffees` recharge les trois snapshots publics — summaries, photos et horaires. Ainsi une publication ou un archivage réconcilie aussi les enfants, même lorsque l'ETag du résumé répond `304`.
...
- **Découverte géographique** : `CoffeeDiscoveryState` ne contient que des intentions UI (texte, tags, ouvert maintenant, photo, tri). `buildCoffeeDiscoveryResults` compose les projections café, photos, horaires et la position GPS en un résultat unique pour la carte comme pour la liste. Ce calcul est local et pur : après réhydratation, recherche, distance, filtres et tri restent utilisables sans réseau. Les horaires inconnus ne sont jamais présentés comme ouverts.
