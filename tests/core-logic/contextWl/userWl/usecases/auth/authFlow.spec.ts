import {
	authListenerFactory,
} from "@/app/core-logic/contextWL/userWl/usecases/auth/authListenersFactory";
import { signInWithProvider, initializeAuth, signOut } from "@/app/core-logic/contextWL/userWl/usecases/auth/authUsecases";
...
import { FakeAuthServerGateway } from "@/tests/core-logic/fakes/FakeAuthServerGateway";
...
describe("auth flow", () => {
	...
	it("refreshes session via backend when tokens about to expire", async () => {
		...
		const state = store.getState().aState;
		expect(typeof (authServer as any).refreshSession).toBe("function");
